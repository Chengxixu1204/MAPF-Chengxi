...
public:
    ...
    int restart_times = 0;
    int agent_selection_mode = 2;
    int pseed = 0;
    double pshuffle = 0;
    vector<int> shortest_path_length;
    int count_pick_shortest_path = 0;
...
private:
    ...
    bool runEECBS();
    bool runCBS();
    bool runPP(int agent_selection_mode = 0);
    bool runPIBT();
    bool runPPS();
    bool runWinPIBT();

    LL getNumPathChoice(int x, vector<TMatrix> &adj_matrix);
    int selectAgent(set<int> &remaining_agents_set, vector<TMatrix> &adj_matrix);
    void modifyAdjMatrix(const Path& path, vector<TMatrix> &adj_matrix);
    int selectAgentBasedonShortestPath(set<int> &remaining_agents_set, ConstraintTable &constraint_table, int mode);
    int selectAgentBasedonMDD(set<int> &remaining_agents_set, ConstraintTable &constraint_table, int mode);
