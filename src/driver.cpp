...
int main(int argc, char** argv)
{
	...
	desc.add_options()
        ...
        ("agtselection", po::value<int>()->default_value(0), "agent selection mode")
        ...
        ("pseed",po::value<int>()->default_value(0), "Random seed for partial shuffling")
        ("pshuffle",po::value<double>()->default_value(0.0), "number or fraction of agents for partial shuffling")
	...
	if (vm["solver"].as<string>() == "LNS")
    {
        LNS lns(instance, time_limit,
                ...
                vm["sipp"].as<bool>(),
                screen, pipp_option);
        lns.agent_selection_mode = vm["agtselection"].as<int>();
        lns.pseed = vm["pseed"].as<int>();
        lns.pshuffle = vm["pshuffle"].as<double>();
        bool succ = lns.run();
        if (succ)
