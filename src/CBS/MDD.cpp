...
bool MDD::buildMDD(ConstraintTable& constraint_table, const SingleAgentSolver* _solver, int threshold)
{
	...
	pairing_heap< Node*, compare<Node::compare_node> > open; open.clear();
	unordered_set<Node*, Node::NodeHasher, Node::eqnode> allNodes_table; allNodes_table.clear();
	open.push(root);
	...
	while (!Q.empty())
	{
		...
		for (auto parent : curr->parents)
		{
			...
			if (parent->mdd_node == nullptr) // a new node
			{
				...
				Q.push_back(parent);
				mdd_size++;
				//if (mdd_size > threshold) return false;
			}
	...
	printf("Open nodes: %d; All nodes: %d\n", open.size(), allNodes_table.size());
	int num_mdd_nodes = 0;
	for (auto & level : levels)
		for (auto & it : level)
			num_mdd_nodes++;
	printf("num of mdd nodes = %d; ", num_mdd_nodes);
	// release memory
	for (auto it : open)
		delete it;
